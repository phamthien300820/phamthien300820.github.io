...
function newMesh() {
  ...
  this.make = function () {
    ...
    if (this.Geometry == 11) {
      mesh = Car(this.Material, this.wheelc, this.bodyc, this.sidec_w, this.sidec_v, this.fontc_w, this.fontc_v);
      console.log(mesh)
...
function newAnimation() {
  ...
  this.runAnimation = function () {
    this.padding = Math.floor(Math.random() * 500) + 100;
    this.radius = Math.sqrt(this.padding ** 2 * 2);
    this.theta = 0;
    this.temp = 0.005;
    if (this.turnOnAnima) {
  ...
  this.meshRotation = function () {
    if (this.rotationMesh) {
      ...
      // this.scale = THREE.Math.clamp(this.scale - 0.005, -1, 1);
      this.scale -= this.temp;
      if (this.scale < 0) this.temp = this.temp * -1;
      if (this.scale > 1) this.temp = -1 * this.temp;
  ...
  this.circleRotation = function () {
    if (this.rotationCircle) {
      ...
      mesh.position.z = Math.cos(this.theta) * this.radius;
      // mesh.position.y = 1/Math.tan(this.theta)*this.radius;
      mesh.position.y += this.cR;
      if (mesh.position.y > 500 || mesh.position.y < 0) {
        this.cR = -1 * this.cR;
      }
  ...
  this.elipRotation = function () {
    if (this.rotationElip) {
      mesh.position.x = Math.sin(this.theta) * this.radius;
      mesh.position.z = 2 * Math.cos(this.theta) * this.radius;
      // mesh.position.y = 1/Math.tan(this.theta)*this.radius;
