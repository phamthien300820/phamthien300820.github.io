...
var texture;
var update;
var lights, pointLight, ambientLight;
var enviro;
var animaObject;
...
function addDatGui() {
  ...
  var wftog = false,
    ptog = false;
  p.add(planeObject, "toggleWireframe")
    .name("Lưới")
    .onChange(function (v) {
      if (v == true) {
        planeObject.makeWireframe();
  ...
  a.add(animaObject, "turnOnAnima")
    .name("Bật chuyển động")
    .onChange(function () {
      animaObject.runAnimation();
    });
  a.add(animaObject, "rotationMesh")
    .name("Bỏ em vào balo")
    .onChange(function () {
      if (animaObject.rotationMesh)
      animaGUI();
    });
  a.add(animaObject, "rotationCircle")
    .name("Di chuyển 360")
    .onChange(function () {
      if (animaObject.rotationCircle)
        animaGUI();
    });
...
function materialzGUI(v) {
  ...
  if (v == 11) {
    matgui
      .addColor(meshObject, "wheelc")
      .name("Bánh")
      .onChange(function () {
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "bodyc")
      .name("Thân")
      .onChange(function () {
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "sidec_w")
      .name("Cửa sổ bên")
      .onChange(function () {
        meshObject.sidec_w = dec2hex_(meshObject.sidec_w);
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "sidec_v")
      .name("Khung bên")
      .onChange(function () {
        meshObject.sidec_v = dec2hex_(meshObject.sidec_v);
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "fontc_w")
      .name("Kính trước")
      .onChange(function () {
        meshObject.fontc_w = dec2hex_(meshObject.fontc_w);
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "fontc_v")
      .name("Khung trước")
      .onChange(function () {
        meshObject.fontc_v = dec2hex_(meshObject.fontc_v);
        meshObject.update();
      });
    matgui
      .addColor(meshObject, "topc")
      .name("Trần")
      .onChange(function () {
        meshObject.update();
      });
...
var animagui;
function animaGUI() {
  animagui = new dat.GUI();
  if (animaObject.rotationMesh == true) {
    var rotate = animagui.addFolder("Xoay và thu nhỏ");
    rotate.add(animaObject, "scalemax", 1, 5).name("Tỉ lệ max");
    rotate.add(animaObject, "temp", 0.001, 0.01).name("Độ giảm");
    rotate.open();
  }
  if (animaObject.rotationCircle == true) {
    var circle = animagui.addFolder("Xoay tròn");
    circle.add(animaObject, "radius", 500, 1500).name("Bán kính");
    circle.add(animaObject, "cR", 0, 30).name("Di chuyển lên");
    circle.add(animaObject, "maxHeight", 500, 1500).name("Độ cao max");
    circle.add(animaObject, "beta", 0, 360).name("Góc quay");
    circle.add(animaObject, "alpha", 1, 5).name("Elip");
    circle.open();
  }

  animagui.add(animaObject, "close").name("CLOSE");
}
